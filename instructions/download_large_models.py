...
try:
    ...
    print(f"\n✓ sentence-t5-large downloaded successfully!")
...
try:
    ...
    print(f"\n✓ Cross-encoder downloaded successfully!")
...
try:
    ...
    print(f"✓ sentence-t5-large working! (embedding size: {len(test_embedding)} dimensions)")
except Exception as e:
    print(f"✗ sentence-t5-large test failed: {e}")

print("\nTesting cross-encoder...")
try:
    test_pairs = [["test query", "test document"]]
    test_score = cross_model.predict(test_pairs)
    print(f"✓ Cross-encoder working! (test score: {test_score[0]:.3f})")
except Exception as e:

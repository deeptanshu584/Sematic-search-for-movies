"""
MRR Evaluation: Enriched Dataset - Model 1 (CHUNKED)
"""
...
print("Testing: Enriched + all-mpnet-base-v2")
model = SentenceTransformer("all-mpnet-base-v2")
texts = df_enriched['combined_text'].tolist()

# Chunked encoding to avoid timeout issues
chunk_size = 1000
embeddings_list = []
...
res = {'dataset': 'Enriched', 'model': 'all-mpnet-base-v2', 'MRR': np.mean(reciprocal_ranks)}
for k in k_values:
    res[f'P@{k}'] = np.mean(precision_at_k[k]) * 100

pd.DataFrame([res]).to_csv('results_enriched_1.csv', index=False)
print("Saved: results_enriched_1.csv")

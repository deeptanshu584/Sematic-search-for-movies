"""
MRR Evaluation: Enriched Dataset - Model 3 (E5 - CHUNKED)
"""
...
texts = ["passage: " + t for t in df_enriched['combined_text'].tolist()]
...
res = {'dataset': 'Enriched', 'model': 'e5-small-v2', 'MRR': np.mean(reciprocal_ranks)}
for k in k_values:
    res[f'P@{k}'] = np.mean(precision_at_k[k]) * 100

pd.DataFrame([res]).to_csv('results_enriched_3.csv', index=False)
print("Saved: results_enriched_3.csv")
